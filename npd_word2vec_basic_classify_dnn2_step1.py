...
model_dir="/abinitio/dev/data/ml/embed9_"+ dir_st
pickle_dir='/abinitio/dev/data/ml/embed_pkl/'
input_pickle_dir='/pub1/dev/PUBLISH/serial_00/ml/'
embed_dir='/abinitio/dev/data/ml/embed_all/embeddings.ckpt'
...
if 1== 1:
	...
	filename ='/abinitio/dev/data/ml/us_ret_final_all_full.dat'
	...
	log.debug("preprocessing.LabelEncoder")
		...
		global_cnt = 0
		temp_input_y=[]
		for i in xrange(len(input_y)):
			if i%split_size == 0:
				global_cnt = global_cnt +1
				log.debug("Pickling .. :" + 'input_y.'+repr(global_cnt)+'.pkl') 	
...
if model == 'svc':
	...
	scores1 = cross_validation.cross_val_score(clf,input_x,input_y,cv=2,n_jobs=-1, scoring='f1_macro')

	print (scores1)
	log.debug("Scores svc:" + str(scores1))
	log.debug("svc Classifcation/Eval Ended")
if model == 'dnn':
	X_train, X_test, y_train, y_test = train_test_split(input_x + 0.01 ,input_y,test_size=0.20, random_state=2)
	del input_x
	...
	classifier.fit(x=X_train, y=y_train, steps=3500,batch_size=128,monitors=[val_monitor])
