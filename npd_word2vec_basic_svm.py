...
if not os.path.isfile(filename):
	dt = pd.read_csv('/abinitio/dev/data/ml/navclass_training_set_US_retailers_and_unclaimed_20160831_v1.dat',header=None,delimiter="|",error_bad_lines=0,names=['poiid','posoutlet','retname','outletdepartmentname','outletclassname','outletbrand','outletdescription','industry','a','b','c','d','industry_name','e','f','g','h'])
	dt = dt[dt['industry_name'] == 'Apparel']
	dt = dt[['retname','outletdepartmentname','outletclassname','outletbrand','outletdescription','industry_name']]
	dt = dt.fillna('')
	dt=dt['retname'].apply(lambda x: x.upper())+' '+dt['outletdepartmentname'].apply(lambda x: x.upper())+' '+dt['outletclassname'].apply(lambda x: x.upper()) + ' '+ dt['outletbrand'].apply(lambda x: x.upper())+' '+dt['outletdescription'].apply(lambda x: x.upper())+' '+dt['industry_name'].apply(lambda x: x.upper())
